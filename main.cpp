...
int points=0;
float bullet_x=0,plane_xmin=0,plane_xmax=0,bullet_y=0;
float time=0;
//Introduction Page
char name[35]="INS VIKRANT";
char college[100]="NMAM INSTITUTE OF TECHNOLOGY,Karkala";
char dept[100]="DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING";
char heading[100]="MADE BY";
char row1[100]="Aanchal Jain (4NM18CS001)                           Under The Guidance Of:";

char row2[100]="Akshatha Shetty (4NM18CS013)                         Prof Sannidhan MS";

char emsg[100]="Press ENTER to start";
char title[50]="                           WARSHIP...!";

char point_dis[15]="Points:";
int screen=0;
...
void frontscreen(void)
{
     ...
     glRasterPos2i(300, 490 );                                               //displays college name
     ...
     glRasterPos2i(250, 430);                                                    //displays dept.
     for(s=0;dept[s]!='\0';s++)
     {
     glColor3f(1.0,1,0.5);
     ...
     glRasterPos2i(300, 360);                                                  //displays project name
     for(s=0;title[s]!='\0';s++)
     {
     ...
     glRasterPos2i(150, 230);                                                    //displays heading of table
     ...
     glRasterPos2i(150,180);                                                    //displays 1st row in table
     for(s=0;row1[s]!='\0';s++)
     {
     glColor3f(1.0, 1.0, 1.0);
     ...
     glRasterPos2i(150,155);                                                   //displays 2nd row in table
     for(s=0;row2[s]!='\0';s++)
     {
     glColor3f(1.0, 1.0, 1.0);
     ...
     glRasterPos2i(400,50);                                                  //displays "enter msg"
     for(s=0;emsg[s]!='\0';s++)
     {
     glColor3f(1.0, 1.0, 1.0);
...
if(bullet_y<=700 && (bullet_x>=plane_xmin && bullet_x<=plane_xmax))
{
...
void crackers()
{
    // if(cf==1)
     ...
     glColor3f(1,0,0);
     glBegin(GL_POLYGON);
     glVertex2f(120+pos,120+pr);
     glVertex2f(130+pos,150+pr);
     glVertex2f(130+pos,150+pr);
     glVertex2f(130+pos,120+pr);
    bullet_x=130+pos;
     bullet_y=130+pr;
     glEnd();
     glFlush();
...
{
glColor3f(1.0,1.0,1.0);
draw_circle(480,645,l);
}

for(l=0;l<=35;l++)
{
   glColor3f(1.0,1.0,1.0);
draw_circle(520,645,l);
draw_circle(545,645,l);
}
for(l=0;l<=20;l++)
{
   glColor3f(1.0,1.0,1.0);
draw_circle(585,645,l);
}
}
void cloud3()
{
...
{
glColor3f(1.0,1.0,1.0);
draw_circle(780,645,l);
}

for(l=0;l<=35;l++)
{
   glColor3f(1.0,1.0,1.0);
draw_circle(820,645,l);
draw_circle(845,645,l);
}
for(l=0;l<=20;l++)
{
   glColor3f(1.0,1.0,1.0);
draw_circle(885,645,l);
}
}
void final_screen()
{
    char message[50]="TIME OUT!!!";
    char point_display[50]="YOUR SCORE:";
    ...
    glPushMatrix();
     ...
     glRasterPos2i(400,300);                                                    //displays college name
     for(s=0;message[s]!='\0';s++)
     {
     glColor3f(1,0,0);
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,message[s]);
     }
        glRasterPos2i(400,400);
     for(s=0;point_display[s]!='\0';s++)
     {
     glColor3f(1.0, 1.0, 1.0);
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,point_display[s]);
     }
    ...
    for(s=0;str[s]!='\0';s++)
     {
     glColor3f(1.0, 1.0, 1.0);
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,str[s]);
     }



     glRasterPos2i(400,200);                                                    //displays heading of table
     for(s=0;thanks[s]!='\0';s++)
     {
     glColor3f(1.0, 1.0, 1.0);
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,thanks[s]);
     }
...
cloud3();
        ...
        glRasterPos2i(850,570);
        int s;

     for(s=0;point_dis[s]!='\0';s++)
     {
     glColor3f(1,0,1);
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,point_dis[s]);
     }
    ...
    for(s=0;str[s]!='\0';s++)
     ...
     glRasterPos2i(850,530);
     char time_dis[10]="Time:";
     for(s=0;time_dis[s]!='\0';s++)
     {
     glColor3f(0,1,1);
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,time_dis[s]);
     }
     char str2[10];
    sprintf(str2, "%f", time);
    for(s=0;str2[s]!='\0';s++)
     ...
     glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24,str2[s]);
     }
     glFlush();
if(cf==1)
    {
